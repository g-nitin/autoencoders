...
class ImageDataset(Dataset):
    def __init__(self, color_dir, perc=1, gray_dir=None, transform=None, target_transform=None):
        """
        ...
        # If `perc` is out of range, then use a value of 1 (100%)
        if perc < 0 or perc > 1:
            perc = 1

        self.names = os.listdir(color_dir)
        self.names = sample(self.names, int(len(self.names) * perc))
